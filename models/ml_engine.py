...
def generate_synthetic_training_data(n_samples: int = 2500) -> pd.DataFrame:
    """
    Generates a realistic synthetic cybersecurity dataset for training ML risk quantification models.
    """
    np.random.seed(1337)
    
    asset_val = np.random.uniform(50000, 10000000, n_samples)
    asset_crit = np.random.randint(1, 6, n_samples)
    is_internet_exposed = np.random.binomial(1, 0.55, n_samples)
    num_users = np.random.randint(10, 500000, n_samples)
    data_sensitivity = np.random.randint(1, 5, n_samples)  # 1: Public, 2: Internal, 3: Confidential, 4: PCI/Secret
    ...
    patch_status = np.random.choice([0, 0.5, 1.0], size=n_samples, p=[0.35, 0.25, 0.40])  # 0: Unpatched, 0.5: In-progress, 1: Patched
    ...
    # Financial Impact ($)
    base_impact = asset_val * (0.05 + 0.05 * asset_crit + 0.04 * data_sensitivity)
    cyber_risk_loss = breach_prob * base_impact * (1.0 + 0.15 * np.log10(np.maximum(10, num_users)))
    
    # Add realistic stochastic noise
    noise = np.random.normal(1.0, 0.12, n_samples)
    target_risk_loss = np.maximum(500.0, cyber_risk_loss * noise)
...
class CyberRiskMLEngine:
    ...
    def predict(self, input_dict: Dict[str, Any]) -> float:
        ...
        return float(np.maximum(0.0, pred))
